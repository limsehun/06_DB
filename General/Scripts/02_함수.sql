...
-- INSTR(문자열 | 컬럼명, '찾을 문자열' [, 찾을 시작 위치 [, 순번]]) (대괄호 및 대괄호 속 내용은 생략하기도 한다)
...
SELECT
		123.456,
		ROUND(123.456), -- 123
		ROUND(123.456, 0), -- 123 (0이 ROUND의 기본값)
...
SELECT 123.456,
		TRUNC(123.456), -- 123
		TRUNC(123.456,1) -- 123.4 (둘째 자리부터 버림ㄴ)
FROM DUAL;

-- 버림(TRUNC), 내림(FLOOR)의 차이점
SELECT
		-123.5,
		TRUNC(-123.5), -- -123
		FLOOR(-123.5) -- -124
...
-- EMPLOYEDE테이블에서 
-- 2010년대에 입사한 사원의 사번, 이름, 입사일을 내림차순으로 조회
...
-- 자릿수 구분(,)
SELECT 123456789, TO_CHAR(123456789,'9,9999,9999')
FROM DUAL;

-- 화폐기호 + 자리수 구분
SELECT 123456789, TO_CHAR(123456789,'L9,9999,9999'),
...
SELECT 
	EMP_NAME,
	TO_CHAR(SALARY+12, 'L9, 9999,9999')
FROM EMPLOYEE;
...
-- '2024-08-16 금요일'
SELECT TO_CHAR(CURRENT_DATE, 'YY-MM-DD DAY')
FROM DUAL;
...
-- 년, 월, 일, 시, 분, 초 는 시간 관련 기호로 인식되지 않아서 오류 발생
