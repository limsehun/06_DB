...
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE -- 1) 부서별 그릅 후 2) 직급별 그룹
ORDER BY DEPT_CODE ASC;
...
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE ASC;
-- 개인 급여가 350만 이상인 사람들의 부서별 평균 (잘몫된 SQL)
...
--> COUNT(ZJFFJAAUD) : 해당 컬럼에 값이 행이 몇 개 인지 카운트
------> NULL 이면 카운트에서 제외
...
-- UNION ALL은 중복도 표시(심봉선,대북혼 2번씩 나옴)
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
