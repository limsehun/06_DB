...
-- SELECT도 DML에 포함되지만 DQL로 분리해서 보기도 한다
...
VALUES(900, '장채현', '001123-2345678', 'jang_ch@kh.or.kr', '01012341234',
...
ROLLBACK;
--> COMMIT되지 않은 DML(INSERT,UPDATE,DELET) 취소하기
	-- == DML 수행 전으로 되돌리기
...
-- 테이블에서 지정하는 조건의 값이 존재하면 UPDATE == KEY 값 중복시 VALUE 수정
-- 조건의 값이 없으면 INSERT됨
...
MERGE INTO EMP_M01 USING EMP_M02 ON(EMP_M01.EMP_ID = EMP_M02.EMP_ID) --중복확인용KEY
WHEN MATCHED THEN --중복확인용KEY가 매치되면(일치하면)
UPDATE SET -- 수정하겠다
