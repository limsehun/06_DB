...
GROUP BY DEPT_TITLE 
HAVING COUNT(*) >= 3;
-- 부서별 인원수가 3명 이상인 부서의 부서명, 인원수 조회
SELECT DEPT_TITLE
FROM EMPLOYEE
LEFT JOIN DEPARTMENT
		ON(DEPT_CODE=DEPT_ID)
...
FROM (SELECT EMP_NAME 이름, SALARY 급여
			FROM EMPLOYEE)
WHERE 급여 >= 4000000; 
			  -- 메인 쿼리 해석 1순위인 FROM절에 작성된 
				-- 서브쿼리 결과 컬럼명 "급여"때문에

				-- 메인쿼리 해석2 순위인 WHERE
				-- 메인쿼리 해석 3순위인 SELECT절도
...
-- 1) 사수에 해당하는 사원 번호 조회
--> MANAGER_ID 컬럼에 작성되에 있는 사번을 가진 사원 == 사수
...
FROM EMPLOYEE "MAIN"
LEFT JOIN DEPARTMENT ON(DEPT_CODE=DEPT_ID)
WHERE EXISTS( -- 서브쿼리 조회 겨과가 있으면 TRUE == 해당 행을 결과에 포함
...
-- 2) DENSE_RANK()OVER
--		값의 크기가 같다면 공동 순위 지정 후 순위 건너뛰기를 하지 않음
SELECT
...
FROM
	...
	FROM EMPLOYEE)
WHERE 
--	ROWNUM BETWEEN 3 AND 7; -- 3행 이상 7행 이하 안됨
	ROWNUM BETWEEN 1 AND 7; -- 1행 이상 7행 이하 이건 됨

-- 해결방법1) ROWNUM 대신 "순위"사용
SELECT
	"순위", EMP_NAME, SALARY
FROM
	(SELECT 
...
WHERE HIRE_DATE >= TO_DATE(2010,'YYYY') AND;
*/
SELECT 
	EMP_ID,
	EMP_NAME,
