 ...
 * 1) FROM 테이블 명;
 * 
 * - 지정된 테이블의 모듵ㄴ 행에서 특정 칼럼만 조회하기
 * 
 * 
 * */

-- EMPLOYEE 테이블에서
-- 모든 행의 이름(EMP_NAME), 급여(SALARY)칼럼 조회
...
-- EMPLOYEE 테이블에 존재하는 모든 행, 모든 칼럼 조회
-- * (asterisk) : "모든", 포함을 나타내는 기호
SELECT * FROM EMPLOYEE;

-- DEPARTMENT 테이블 에서 부서코드, 부서명 조회
SELECT 
	DEPT_ID,
	DEPT_TITLE
FROM DEPARTMENT;
...
/* DUAL (DUmmy tAbLe)
 * - 가짜 테이블(임시 테이블)
 * - 조회하려는 데이터가 실제 테이블에 존재하는 데이버가 아닌 경우에 사용
 * 
 * 
 **/
...
-- TO_DATE('날짜','패턴');
-- -> '날짜문자열'을 '패턴' 형식으로 해석하여 DATE타입으로 변환하는 함수
...
-- EMPLOYEE 테이블에서 모든 사원의 사번, 이름, 입사일, 현재까지 근무 일 수 조회
SELECT EMP_ID,EMP_NAME,HIRE_DATE,CURRENT_DATE - HIRE_DATE,
...
/*- 별칭 지정 방법
 * 1) 컬럼명 AS 별칭 : 문자O, 띄어쓰기 X, 특수문자O
 * 
 * 2) 컬럼명 AS "별칭" : 문자O, 띄어쓰기 O, 특수문자O
 * 
 * 		-> AS는 생략 가능
 * 
 * - ORACLE에서 ""의 의미 -> ""내부에 작성된 글자 그대로를 인식해라
 * 		EX)  -  : 빼기
 * 			  "-" : - 모양 글자
 * 
 * ** ORACLE에서 문자열은 '' (홑따옴표)
...
SELECT 
	EMP_ID AS 사번, -- 컬럼명AS별칭
	EMP_NAME 이름, -- 컬럼명 별칭 (AS 생략)
	HIRE_DATE AS "입사한 날짜", -- 컬럼명 AS 별칭명
...
/* 컬럼명 =NULL , 컬럼명 != NULL (X)
 * 
 * -> =, != 같은 비교연산은 컬럼에 저장된 값을 비교하는 연산
 * 
 * -> ORACLE DB에서 NULL은 값이 아니라 값이 존재하지 않다(빈칸)는 뜻 == 저장된 값이 없다.
 * 
 * *************************
...
-- EMPLOYEE 테이블에서 BONUS 컬럼이 있는 사원 조회
...
-- AND(그리고) : 두 조건식의결과가 TRUE인 경우만 TRUE
		--> 두 조건을 모두 만족하는 행만 조회 결과에 포함

-- OR(또는) : 두 조건중 하나라도 TRUE인 경우 TRUE
		-->두 조건중 하나라도 만족하는 행만 조회 겨과에 포함

-- EMPLOTEE 테이블에서 부서코드가 'D6'인 사원 중
...
-- EMPLOTEE 테이블에서 급여가 300만 이상 500만 미만인 사원의 사번 이름 급여 조회
...
-----------------------------------
/* 컬럼명 BETWEEN (A) AND (B) */
-- 컬럼 값이 "A 이상", "B이하"인 경우 TRUE(조회하겠다)
-----------------------------------

-- EMPLOTEE테이블에서 급여가 400만 이상 600만 이하인 사원의 이름 급여 조회
...
WHERE HIRE_DATE 
 ...
 -- EMPLOYEE테이블에서 부서코드가 'D5', 'D6', 'D9'인 사원의 사번 이름 부서코드 조회
 
 SELECT EMP_ID , EMP_NAME , DEPT_CODE 
 FROM EMPLOYEE
 WHERE DEPT_CODE ='D5' 
 		OR DEPT_CODE ='D6' 
 		OR DEPT_CODE ='D9'; --12행 조회됨
 		
---------------------------------------
 /*컬럼명 IN (값1,값2,값3 .......)*/
-- 칼럼의 값이 IN() 내에 존재하면 TRUE --> 조회 결과에 포함
...
---------------------------------------
 /*컬럼명 IN (값1,값2,값3 .......)*/
-- 칼럼의 값이 NOT IN() 내에 존재하지 않으면 TRUE --> 조회 결과에 포함
-- == 연속으로 OR 연산을 작성한 것과 같은 효과
---------------------------------------
 ...
 * '%' (포함)
 * 
 * EX) '%A'  : 문자열 앞 부분은 어떤 문자든 포함 할 수 있지만 마지막은 A로 끝나는 문자열
 * 		 'A%'  : A로 시작하는 문자열
 * 		 '%A%' : A가 포함된 문자열
 * 
 * '_' (글자수)
 * 
 * EX) 'A___' : A로 시작하고 그 뒤에 3글자만 있는 문자열
 * 						 ex) ABCD (O), ABCDE(X)
...
SELECT EMP_ID, EMP_NAME
FROM EMPLOYEE
WHERE EMP_NAME LIKE '%하%'; --OR EMP_NAME LIKE '%수%';
 ...
 * 
 * [작성법]
 *  3) SELECT 컬럼명 (특정 컬럼만 선택해서 볼게)
 *  1) FROM 테이블명 (어떤 테이블에서)
 *  2) WHERE 조건식 (어떤 조건을 만족하는 행만 선택(조회))
 *  4) ORDER BY 	(조회된 결과를 정렬할게)
 * 	   컬럼명 | 별칭 | 컬럼순서(숫자)
 *     [ASC / DESC]        (오름차순/내림차순)
...
--ORDER BY SALARY DESC;
ORDER BY 3 DESC;
 --> 먼저  해석된 SELECT절에 작성된 컬럼 순서를 이용해서
 --  정렬 기준이 될 컬럼을 지정할 수 있다.(단, 수정 할 일이 잦아서 자주 쓰진 않는다.)
...
SELECT EMP_ID "사번",
			 ...
			 JOB_CODE "직급코드"
FROM EMPLOYEE
WHERE JOB_CODE/*여기엔 별칭 사용 못 함*/ IN ('J4','J5','J6')
ORDER BY "이름" ASC;
...
SELECT EMP_NAME 이름, DEPT_CODE 부서, JOB_CODE 직급
FROM EMPLOYEE
ORDER BY 부서 ASC, 직급 ASC, 이름 ASC;
