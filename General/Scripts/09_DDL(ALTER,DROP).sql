...
ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE IS NOT NULL; -- NULL 허용 X
...
-- ALTER == DDL -> TRANSACTION을 밀고 DB로 직행!!!!!!!
...
--> 결론 : DML/DDL 혼용해서 사용하지 말자!!! -> 무결성 침해된다제
...
-- 해결 방법1) 참조하는 자식 테이블(TB2)을 먼저 삭제하거나 FK 제약조건을 삭제

-- 해결 방법1) CASCADE CONSTRAINTS 옵션 사용하기
--> 테이블 삭제 시 연결 되어 있는 FK 제약조건을 같이 삭제 시킴
